...
function testAction.onUse(player, item, fromPosition, target, toPosition, isHotkey) -- now we can design the action itself
    ...
    if player:removeMoney(c.price) then
        ...
        player:sendCancelMessage("Your purshase was successful. You paid 4000 gold.")
        fromPosition:sendMagicEffect(CONST_ME_HEARTS)
    else
        player:sendCancelMessage("You do not have enough money. You need 4000 gold.")
        fromPosition:sendMagicEffect(CONST_ME_POFF)
    end
    return true
end

testAction:aid(46008) -- the action id for lever
